...
Download `MSFSResumeSetup-0.4.4.exe` from [Releases](https://github.com/unicornfss/msfs_resume/releases) and run it. The installer does not need administrator rights.
...
- Spawn near the suggested airport (copy the ICAO), set fuel, QNH and sim time, configure the aircraft, then click **Restore**.
- The last few restore points are kept if you need an earlier one.
- Starts in the system tray; a reminder can be turned off. You get a popup for an incomplete flight or a sim crash.
- SimBrief username, fuel tolerance, tray start and always-on-top are under **File → Settings**.
...
- `dist\MSFSResumeSetup-0.4.4.exe` — Windows installer (if Inno Setup is installed)
